...
public class DiamondSquare2D extends BaseTerrain2D {
	...
	public DiamondSquare2D(double[][] start) {
		init(start);
	}
	...
	/*
	 * t = { 3 6 0 3 0 3 9 6 0 } normalize(t, 9, 1) -> t = {1/3, 2/3, 0, 1/3, 0,
	 * 1/3, 1, 2/3, 0} normalize(t, 9, 0) -> t = {0 0 0 ... 0}
	 */
	...
	public void diamond(int sX, int sY, int width, double startAmp) {
		if (!forceStay[sX + width / 2][sY + width / 2])
			t[sX + width / 2][sY
					+ width / 2] = (t[sX][sY] + t[sX + width][sY] + t[sX][sY + width] + t[sX + width][sY + width]) / 4;
		if (!positiveOnly)
	...
	public void square(int sX, int sY, int width, double startAmp) {
		...
		if (sX - width / 2 < 0)
			...
			t[sX][sY] = (t[sX][sY + width / 2] + t[sX][sY - width / 2] + t[sX + width / 2][sY] + t[sX - width / 2][sY])
					/ 4;
		if (!positiveOnly)
