...
public class OctaveNoise2D extends BaseTerrain2D {

	public OctaveNoise2D(double seed) {
		// this.seed = seed;
		random = new Random((long) seed);
	}

	public double[][] makePerlinNoise(int width, int height, double startAmp, double startFreq, double persistence,
			double ampFreqRatio, int times) {
		double[][][] averageLater = new double[times][width][height];
		for (int i = 0; i < times; i++) {
			averageLater[i] = generateNoise(width, height, startAmp * Math.pow(ampFreqRatio, i),
					(int) (startFreq / Math.pow(ampFreqRatio, i)));
		}
