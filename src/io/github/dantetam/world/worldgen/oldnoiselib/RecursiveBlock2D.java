...
public class RecursiveBlock2D extends BaseTerrain2D {
	...
	public RecursiveBlock2D(long seed) {
	...
	// Returns true if there are fewer than 3 zeroes and the data is flat (low std
	// dev)
	...
	public double[][] heightMap(int widthBlock) {
		...
		int row = 0;
		int col = 0; // Keep track of position in table
	...
	public void diamond(int sX, int sY, int width, double startAmp) {
		if (sX + width >= terrain.length || sY + width >= terrain.length)
			return;
				...
				for (int j = 0; j < (int) Math.pow(2, i); j++) {
					...
					if (size >= 16) {
						startIsland(size, size, size, (int) (radius * Math.cos(angle)), 100,
								(int) (radius * Math.sin(angle)), (int) (Math.random() * 2 + 1));
					}
				}
	...
	public void printTable(double[][] t) {
		for (int r = 0; r < t.length; r++) {
			for (int c = 0; c < terrain[0].length; c++) {
				...
				if (height < 10) {
					// Attempt to program the equivalent of C/C++'s "%02d" to align the data
					// correctly
	...
	public void terrain(Entity en, int times) {
		for (int i = 0; i < times; i++) {
			...
			if (clone.sizeX <= 3 || clone.sizeY <= 3 || clone.sizeZ <= 3 || clone.getMass() < 30) {
				return;
			}
