...
public class VBOLoader {
	...
	/**
	 * If the texture is already loaded, return the saved the texture ID,
	 * otherwise load the texture using standard file IO and OpenGL buffers
	 * @param overrideStoredFileStr If true, potentially reload a texture even when the file name has been seen before,
	 * 								otherwise, use the cached VAO texture ID.
	 */
	public static int loadTexture(String fileName, boolean overrideStoredFileStr, boolean advancedRender)
	{
        ...
        if (advancedRender) {
	        ...
	        GL11.glTexParameterf(GL11.GL_TEXTURE_2D, EXTTextureFilterAnisotropic.GL_TEXTURE_MAX_ANISOTROPY_EXT, anisotropicFilterAmount); 
        }
