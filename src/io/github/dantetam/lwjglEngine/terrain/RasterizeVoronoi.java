...
public class RasterizeVoronoi {
	...
	/**
	 * Return a new double[][] arr representing the pixels of a new blend map, i.e.
	 * a grayscale terrain.
	 * 
	 * @param voronoi
	 * @param voronoiBounds
	...
	private static double[][] getPixelRasterGridFromVoronoi(List<JSite> voronoi, Rectangle2D.Double voronoiBounds,
		...
		// Create this geometric data all in advance for use in overlapping calculations
		// later
		...
		for (int rasterCol = 0; rasterCol < cols * rasterColTimes; rasterCol++) {
			for (int rasterRow = 0; rasterRow < rows * rasterRowTimes; rasterRow++) {
				double voronoiSpaceX = ((rasterCol + 0.5) / rasterResults.length) * voronoiBounds.width
						+ voronoiBounds.x;
				double voronoiSpaceY = ((rasterRow + 0.5) / rasterResults[0].length) * voronoiBounds.height
						+ voronoiBounds.y;
				Point2D inspectPoint = new Point2D(voronoiSpaceX, voronoiSpaceY);

				boolean foundMatch = false;

				// Search for the polygon containing this calculated point in the 'Voronoi
				// space'
				// Note that because of the noise generation algorithms, there is no guarantee
				// either way between
				// point is closest to centroid x <-> this point 'belongs' to the modified
				// Voronoi polygon.
				// This was a guaranteed if and only if relationship for normal Voronoi
				// diagrams.
				...
				for (Point2D neighborCandidate : nearestNeighbors) {
					...
					if (polygonCandidate.contains(inspectPoint)) {
						...
						double avgBiome = 0;
						double[] vertexWeights = PolygonInterpolation.getPolyCoords(polygonCandidate,
								new Point2D(voronoiSpaceX, voronoiSpaceY), fWeights);
	...
	private static int[] getBiomePolyVoronoiSpace(PolygonSimple polygon, Rectangle2D.Double voronoiBounds,
			int[][] biomesGuide) {
	...
	// The process of creating a random Voronoi tessellation and then performing
	// Lloyd relaxation,
	...
	// Particularly, E(Voronoi polygon edges) ~ 6 and Var(Voronoi polygon edges) ~
	// 1.78.
	// See "Statistical Distributions of Poisson Voronoi Cells...", Tanemura, 2002,
	// and also
	 ...
	 * @return
	 */
	public static BufferedImage generateVoronoiTesselPoly(double[][] terrain, int[][] biomes, int rasterRows,
			int rasterCols, boolean transposed) {
		...
		Rectangle2D.Double voronoiBounds = new Rectangle2D.Double(0, 0, rasterCols * terrain[0].length,
				rasterRows * terrain.length);
	...
	public static void main(String[] args) {
		/*
		 * Point2D topLeftBound = new Point2D(0, 0); Point2D bottomRightBound = new
		 * Point2D(1600, 1600); double averageDistance = 50; int lloydRelaxationTimes =
		 * 4; List<JSite> voronoi = VoronoiLibrary.voronoiLib(topLeftBound,
		 * bottomRightBound, averageDistance, lloydRelaxationTimes); Rectangle2D.Double
		 * voronoiBounds = new Rectangle2D.Double(0, 0, 1600, 1600);
		 * 
		 * //voronoi = ParallelogramNoise.parallelogramNoiseOnVoronoi(voronoi, 0.15,
		 * 0.15);
		 * 
		 * long seed = 500L; //System.currentTimeMillis(); //500L
		 * 
		 * int len = 32; double[][] temp = DiamondSquare.makeTable(50, 50, 50, 50, len +
		 * 1); temp[temp.length / 2][temp[0].length / 2] = 200; temp[0][temp[0].length /
		 * 2] = 50; temp[temp.length - 1][temp[0].length / 2] = 50; temp[temp[0].length
		 * / 2][0] = 50; temp[temp[0].length / 2][temp.length - 1] = 50; BaseTerrain map
		 * = new DiamondSquare(temp); map.seed(seed); double[][] terrain =
		 * map.generate(new double[] { 0, 0, len, 40, 0.7 }); double cutoff = 100;
		 * 
		 * //int rows = 66, cols = 66, rasterRows = 10, rasterCols = 10; int rasterRows
		 * = 40, rasterCols = 40; GameTerrainGeneration gameTerrainGen = new
		 * GameTerrainGeneration(seed); int[][] biomes =
		 * gameTerrainGen.getAssignedBiome(terrain, cutoff); double[][] rasterData =
		 * getPixelRasterGridFromVoronoi(voronoi, voronoiBounds, biomes, rasterRows,
		 * rasterCols);
		 * 
