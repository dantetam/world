...
public class ParallelogramNoise {
	...
	/*
	 * Return noise fixed around a line, constrained to a quadrilateral.
	 * This uses the "diamond-square" method of bending a line segment by moving 
	 * one of its midpoints up or down a random amount, and then applying
	 * the same recursion to the two new line segments. The recursion stops
	 * at a base case of some desired granularity. 
	 */
	...
	public static double[] divideLineSegment(Point2D p1, Point2D p2, double amp, double splitLimit) {
		...
		if (Math.random() < 0.5) extendAngle -= Math.PI;
		...
		System.out.println("Angle to extend: " + extendAngle + " " + Math.toDegrees(extendAngle));
		System.out.println("Split proportion: " + randomSplit);
		System.out.println("(" + extendX + "," + extendY + ")");
	...
	/**
	 * Recursively split any chain of vertices of size n into 2n - 1, and repeat the process for each new chain
	 * @param chainEdges
	 * @param amp
	 ...
	 * @return
	 */
	public static List<Point2D> parallelogramBoundNoise(List<Point2D> chainEdges, double amp, double ampReduceFactor, double splitPercentage, double cutoffSplitDist) {
		...
		for (int i = 0; i < chainEdges.size() - 1; i++) {
			...
			if (chainEdges.get(i).distance(chainEdges.get(i+1)) >= cutoffSplitDist) {
				double[] splitSegment = divideLineSegment(chainEdges.get(i), chainEdges.get(i+1), amp, splitPercentage);
		...
		return parallelogramBoundNoise(newChainEdges, amp * ampReduceFactor, ampReduceFactor, splitPercentage, cutoffSplitDist);
	}
	
	public static List<JSite> parallelogramNoiseOnVoronoi(List<JSite> originalVoronoi, double ampDistUnits, double cutoffDistUnits) {
		...
		for (Entry<Edge, List<Integer>> entry: sharedEdgesMap.entrySet()) {
			...
			if (polygonIndices.size() == 2) {
				...
				List<Point2D> newChain = parallelogramBoundNoise(beginningChain, dist * ampDistUnits, 0.65, 0.45, dist * cutoffDistUnits);
	 ...
	 * @return
	 */
	public static PolygonSimple polygonNoiseAllEdges(PolygonSimple polygon, double ampDistUnits, double cutoffDistUnits) {
		...
		for (List<Point2D> chain : newEdgeChains) {
			chain.remove(0); //Do not double count these vertices, which are the points where chains join together
